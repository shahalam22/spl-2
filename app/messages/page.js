...
import { fetchChatUsers, fetchMessages } from '@/redux/features/messageSlice';

// Connect to the Socket.IO server
const socket = io("http://localhost:5000");

function Messages({anotherUserId}) {
  ...
  const currentUserId = parseInt(searchParams.get('currentUserId'))|| 14;
  const receiverUserId = parseInt(searchParams.get('anotherUserId')) || 16;
  
  useEffect(() => {
    dispatch(fetchMessages({ userId: currentUserId, anotherUserId: receiverUserId }));
    // dispatch(fetchChatUsers({userId}));
  });
  // console.log(currentUserId, receiverUserId);
  const prevmessages = useAppSelector(state => state.messages.messages);
  ...
  return (
    <>
        ...
        <div className="flex gap-2 flex-col md:flex-row">
          ...
          <div className="w-auto md:w-[100%] flex flex-col gap-4 m-5 p-5 bg-gray-100 rounded-lg">
