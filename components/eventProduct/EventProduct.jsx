...
function EventProduct({ product, variant }) {
  ...
  // Use local state for realtime bid amount updates.
  const [currentBidAmount, setCurrentBidAmount] = useState(product.bidAmount);

  const userId = useAppSelector((state) => state.auth.user?.user_id);
  const eventId = useAppSelector((state) => state.events.currentEvent.event_id);

  // Join the event room and listen for bid updates
  useEffect(() => {
    const socket = io("http://localhost:5000");
    socket.emit("joinEvent", { eventId, userId });
    socket.on("bidAmountUpdated", (updatedPost) => {
      if (updatedPost.post_id === product.post_id) {
        setCurrentBidAmount(updatedPost.bidAmount);
      }
    });
    return () => {
      socket.disconnect();
    };
  }, [eventId, product.post_id, userId]);
    ...
    <>
      <div className='flex justify-between py-4 px-6 rounded-2xl bg-white shadow-md' onClick={toggleCardDetails}>
        ...
        <div className='flex gap-10 items-center'>
          <div className='flex gap-4'>
            <p>
              <span className='text-cyan-500 font-semibold'>Ask : </span>{product.price}
            </p>
            <p>
              <span className='text-red-500 font-semibold'>Bid : </span>{currentBidAmount}
            </p>
          </div>
          <div className='flex gap-2'>
            {variant === 'viewcard' && (
      ...
      {showCard && <SingleResourcePage onClose={toggleCardDetails} resource={product} isEditCard={variant === "editcard"} />}
      {showEdit && <ResourceForm onClose={toggleEditDetails} data={product} />}
      {showBid && (
        <div className='fixed inset-0 z-10 bg-black bg-opacity-50 backdrop-blur-sm flex justify-center items-center' onClick={toggleBidDetails}>
          <BidDialogue
            onClose={toggleBidDetails}
            userId={userId}
            eventId={eventId}
