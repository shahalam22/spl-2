...
function ResourceCard({variant, category, condition, exchangeOption, location, title, description, price, image}) {
  ...
  return (
    <>
...

        {
          variant === 'editcard' && (
            <div className="card-edit-footer" onClick={(e) => e.stopPropagation()}>
              <div className='w-[78%]' onClick={showEditDetails}>
      ...
      {
        showCard && <SingleResourcePage onClose={closeCardDetails}/>
      }
      {
        showEdit && <ResourceForm onClose={closeEditDetails}/>
      }
    </>
  );
}
